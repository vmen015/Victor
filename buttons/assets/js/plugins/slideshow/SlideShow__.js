...
SlideShow.defineTransitions({
	...
	fade: function(data){
		data.previous.set('tween', {duration: data.duration, transition: data.instance.options.easing}).fade('out');
		return this;
	},

	crossFade: function(data){
		data.previous.set('tween', {duration: data.duration, transition: data.instance.options.easing}).fade('out');
		data.next.set('tween', {duration: data.duration, transition: data.instance.options.easing}).fade('in');
		return this;
	},

	fadeThroughBackground: function(data){
		var half = data.duration / 2;
		data.next.set('tween', {duration: half, transition: data.instance.options.easing}).fade('hide');
		data.previous.set('tween',{
			duration: half,
			transition: data.instance.options.easing,
...
(function(){
	...
	function go(type, styles, data){
		var tweenOptions = {duration: data.duration, unit: '%', transition: data.instance.options.easing};
		if (type == 'blind') {
