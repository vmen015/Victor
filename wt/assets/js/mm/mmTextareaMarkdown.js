/* mmTextareaMarkdown */
var mmTextareaMarkdown = new Class({
	...
	, property: 'mmTextareaMarkdown'
	, initialize:function(element, options) {
mm.log('mmTextareaMarkdown initialize');		
...
var mooWMD={
	...
	Config:{
		 ...
		 */
		helpLink: "http://moowmd.awardspace.info/",
	...
	SpritedButtonRow: new Class({
		 ...
		 */
		build: function(){
			var buttonBar = document.id('wmd-button-bar'+this.postfix);//TODO in later version, user will have to only put a textarea.
		...
		doBlockquote: function(chunk, postProcessing, useDefaultText){
			...
			if(/^(?![ ]{0,3}>)/m.test(chunk.selection)){
				chunk.wrap(chunk, mooWMD.Config.lineLength - 2);
		...
		setUndoRedoButtonStates:function(){
			if(this.undoMgr){
				this.setupButton(document.id("wmd-undo-button"+this.postfix), this.undoMgr.canUndo());
				this.setupButton(document.id("wmd-redo-button"+this.postfix), this.undoMgr.canRedo());
			}
		},
		...
		setupButton: function(button, isEnabled) {
			...
			var disabledYShift = "-20px";
			var highlightYShift = "-40px";
			button=document.id(button);//for IE
...
//IE users can go screw themselves
/*
...
*/			
		...
		doClick$: function(button){
			return this.doClick(document.id(button+this.postfix));
		},
		doClick: function(button){
			this.input.focus();
		...
		addButtonActionsToInputBox: function(E){
			...
			if (E.control || E.meta) {
				...
				switch(keyCodeStr) {
					case "b":
						this.doClick(document.id("wmd-bold-button"+this.postfix));
						break;
					case "i":
						this.doClick(document.id("wmd-italic-button"+this.postfix));
						break;
					case "l":
						this.doClick(document.id("wmd-link-button"+this.postfix));
						break;
					case "q":
						this.doClick(document.id("wmd-quote-button"+this.postfix));
						break;
					case "k":
						this.doClick(document.id("wmd-code-button"+this.postfix));
						break;
					case "g":
						this.doClick(document.id("wmd-image-button"+this.postfix));
						break;
					case "o":
						this.doClick(document.id("wmd-olist-button"+this.postfix));
						break;
					case "u":
						this.doClick(document.id("wmd-ulist-button"+this.postfix));
						break;
					case "h":
						this.doClick(document.id("wmd-heading-button"+this.postfix));
						break;
					case "r":
						this.doClick(document.id("wmd-hr-button"+this.postfix));
						break;
					case "y":
						this.doClick(document.id("wmd-redo-button"+this.postfix));
						break;
					case "z":
						if(E.shift) {
							this.doClick(document.id("wmd-redo-button"+this.postfix));
						}else{
							this.doClick(document.id("wmd-undo-button"+this.postfix));
						}
						break;
					default:
						return;
	...
	Prompt: new Class({
		...
		createBackground: function(){
			...
			document.id(document.body).adopt(this.background);
		...
		createDialog: function(){
			...
			document.id(document.body).addEvent("keydown", this.checkEscape);
	...
	Editor: new Class({
		...
		prepareTextForSubmit: function(){
			if (this.input.form) {
				...
				Form.addEvent('submit',this.convertToHtml.bind(this));
			}
	...
	PreviewMgr: new Class({
		...
		makePreviewHtml: function(){
			...
			if (this.converter) {
				...
				// strike test:
				text = text.replace(/(([^:]\/{2})(.+))/gm, "<strike>$3</strike>");							
	...
	WMD: new Class({
		...
		start: function(){
			...
			for(var i=0;i<len;i++){
				...
				var input=this.cleanP(document.id(this.json[i].input));//A bug with Hebrew, I will NOT dive into showdown. It seems <p></p>tags
															 //are making a problem with hebrew. I will remove them.
				var postfix=this.json[i].postfix;
				var panel={
						preview: document.id(preview),
						output: document.id(output),
						input: input
				};
